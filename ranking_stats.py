...
def process_row(row, top_bert, top_tfidf, vectorizer):
    ...
    return row.name, bert_similarity, tfidf_similarity
...
if not os.path.exists(f"feature_data_{cp}_new_sim.csv"):
    ...
    for qid in list(texts_df.query_id.unique()):
        ...
        for index, row in subset_df.iterrows():
            futures.append(executor.submit(process_row, row, top_bert, top_tfidf, vectorizer))

    # Retrieve results and update DataFrame
    for future in tqdm.tqdm(futures):
        index, bert_sim, tfidf_sim = future.result()
        texts_df.at[index, "top_bert_sim"] = bert_sim
        texts_df.at[index, "top_tfidf_sim"] = tfidf_sim

    final = texts_df.merge(orig, on=["query_id", "creator", "username"], how="left")
    final.to_csv(f"feature_data_{cp}_new_sim1.csv", index=False)
